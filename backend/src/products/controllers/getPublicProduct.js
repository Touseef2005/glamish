...
import getPublicData from "../services/publicProducts.js"
import ProductModel from "../models/index.js"
import { fallbackHandler } from '../../../utils/index.js'

const getPublicProductController = async (req, res) => {
    ...
    try {
        ...
        const allowedFilters = ["name", "category", "brand", "isFeatured"];
      ...
      return [key, { $regex: String(value), $options: "i" }];
    })
        );
        ...
        const dataPaginate = await fallbackHandler({
            ...
            dbFetchFunction: async () => {
                const products = await getPublicData({ limit: recordsLimit, skip: recordsSkip, search });
...
export default getPublicProductController;
