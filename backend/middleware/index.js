...
export async function redisCacher(req, res, next) {
    const response = new Response(res);
    const { id } = req.params;

    try {

        const cachedData = await response.redis.get(id);
        if (cachedData) {
            // console.log("✅ Cache Hit");
            return response.success(JSON.parse(cachedData), "Data fetched successfully from Cache", 200);
        }
